...
using System.Linq;
    ...
    public class TargetFramework
    {
        ...
        {
            string directoryPath = "/storage/emulated/0/Android/data/app.SMAPIStardew/Mods"; // 替换为你要扫描的根目录路径
            string newTargetFramework = "net8.0"; // 设置新的目标框架版本
            ...
            foreach (var assemblyPath in assemblyFiles)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine($"程序集 '{assemblyPath}' 已是目标框架 '{newTargetFramework}'，跳过修改。");
                        continue; // 如果目标框架已经是期望的版本，跳过
                    }
                    if (IsStardewValleyAssembly(assemblyDefinition))
                    {
                       Console.WriteLine($"程序集 '{assemblyPath}' 是 StardewValley 程序集，跳过修改。");
                    ...
                    if (File.Exists(assemblyPath))
                    {
                        ...
                        Console.WriteLine($"原始程序集 '{assemblyPath}' 已删除。");
                    }

                    // 将修改后的程序集重命名为原文件名
                    File.Move(modifiedAssemblyPath, assemblyPath);
                    Console.WriteLine($"修改后的程序集已重命名并覆盖为 '{assemblyPath}'");
                }
                catch (Exception ex)
                {
        ...
        static void UpdateTargetFramework(AssemblyDefinition assemblyDefinition, string targetFramework)
        {
            ...
            if (metadata != null)
            {
                ...
                {
                    Console.WriteLine("目标框架已经是目标版本，不需要更新。");
                    return;
                }
            ...
            else
            {
                ...
                Console.WriteLine($"添加新的目标框架属性 '{targetFramework}'");
            }
        }
