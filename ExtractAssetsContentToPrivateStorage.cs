...
namespace SMAPIStardewValley
{
    public class ExtractAssetsContentToPrivateStorage
    {
        ...
        public static async Task ExtractAssetsContentAsync()
        {
            try
            {
                ...
                string apkFilePath = Path.Combine(MainActivity.GetPrivateStoragePath(), "stardewvalley.apk");
            ...
            catch (Exception ex)
            {
              ...
              public static async Task ExtractapksAssetsContentAsync()
        {
            try
            {
                ...
                if (!File.Exists(apkFilePath))
                {
                  //666  Toast.MakeText(MainActivity.mainActivity, "APK 文件不存在", ToastLength.Long).Show();
                    return;
                }
