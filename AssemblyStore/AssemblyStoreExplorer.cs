...
namespace Xamarin.Android.AssemblyStore
{
    class AssemblyStoreExplorer
    {
        ...
        public AssemblyStoreExplorer(ZipFile archive, string basePathInArchive, Action<AssemblyStoreExplorerLogLevel, string>? customLogger = null, bool keepStoreInMemory = false)
        {
            logger = customLogger;
            this.keepStoreInMemory = keepStoreInMemory;
        ...
        void ReadStoreSetFromArchive(string baseName, string archivePath, string extension)
        {
            ...
            using (ZipFile archive = new ZipFile(fs))
            {
                ReadStoreSetFromArchive(archive, basePathInArchive);
            }
        }

        void ReadStoreSetFromArchive(ZipFile archive, string basePathInArchive)
        {
            foreach (ZipEntry entry in archive)
            {
                if (!entry.Name.StartsWith(basePathInArchive, StringComparison.Ordinal))
                {
                    continue;
                }
                ...
                {
                    // Extract the entry to the MemoryStream
                    using (Stream entryStream = archive.GetInputStream(entry))
                    {
                        entryStream.CopyTo(stream);
                    }

                    // Set the position of the stream back to the beginning for reading
                    stream.Seek(0, SeekOrigin.Begin);

                    if (entry.Name.EndsWith(".blob", StringComparison.Ordinal))
                    {
                        AddStore(new AssemblyStoreReader(stream, GetStoreArch(entry.Name), keepStoreInMemory));
                    }
                    else if (entry.Name.EndsWith(".manifest", StringComparison.Ordinal))
                    {
